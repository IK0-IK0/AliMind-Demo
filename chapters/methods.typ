...
=== NLP-Based Construct Inference

The inference of TPB constructs and TTM stages from natural language rests on two complementary theoretical foundations. First, the Cognitive Aspects of Survey Methodology (CASM) framework establishes that answering any survey question involves four cognitive stages: comprehension, retrieval from memory, judgment formation, and response mapping @tourangeau2000psychology. The first three stages remain identical regardless of response format; only the final stage differs between structured and open-ended responses. Whether a respondent rates their perceived behavioral control as "3" on a Likert scale or states "I don't have time to cook" in conversation, they have engaged the same cognitive processes. The difference lies only in how the internal judgment is expressed.

Second, distributional semantics provides the computational foundation for extracting these latent judgments from text. The distributional hypothesis holds that words appearing in similar contexts carry similar meanings @harris1954distributional, enabling vector representations to capture semantic relationships. Recent research demonstrates that language model embeddings encode psychological constructs in ways that align with human psychometric assessments, with semantic similarity in embedding space corresponding to similarity in psychological meaning. This allows NLP models to map natural language expressions to the same latent constructs measured by validated instruments.

Empirical support for construct equivalence comes from qualitative TPB research demonstrating that open-ended elicitation interviews reliably capture belief structures underlying attitudes, subjective norms, and perceived behavioral control @francis2004constructing @downs1999eliciting, and from TTM research showing that stage classification can be reliably derived from open-ended interviews @prochaska2005transtheoretical. Studies further demonstrate that conversational formats yield richer responses while maintaining construct consistency @schuman1996questions @geer1988what @bishop1988changing.

The NLP inference models developed in this research leverage both principles: CASM validates that conversational responses contain the same psychological constructs as structured assessments, while distributional semantics enables computational extraction of these constructs from text. The paired training data where participants provide both open-ended explanations and corresponding Likert-scale ratings enables supervised learning of this mapping function, ensuring that inferred scores reflect the same cognitive judgments captured by validated psychometric instruments.
