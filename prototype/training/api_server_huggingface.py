"""
Flask API server for TPB/TTM inference using HuggingFace Hub
Loads models from HuggingFace instead of local files
"""
...
HUGGINGFACE_USERNAME = "franzzzzzzzzz"

# Load models on startup
logger.info(f"Loading models from HuggingFace user: {HUGGINGFACE_USERNAME}")
logger.info("This may take a few minutes on first run (downloading models)...")
logger.info("Subsequent runs will be fast (models are cached locally)")

try:
    tpb_inference = TPBInferenceFromHub(HUGGINGFACE_USERNAME)
    ttm_inference = TTMInferenceFromHub(HUGGINGFACE_USERNAME)
    logger.info("✓ All models loaded successfully!")
except Exception as e:
    logger.error(f"✗ Failed to load models: {e}")
    logger.error("Make sure models are uploaded to HuggingFace and are public")
    raise
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'message': 'API is running', 'source': 'huggingface'})
...
if __name__ == '__main__':
    # Run server
    print("\n" + "="*60)
    print("TPB/TTM Inference API Server (HuggingFace)")
    print("="*60)
    print("\nEndpoints:")
    ...
    print(f"\nLoading models from: {HUGGINGFACE_USERNAME}")
