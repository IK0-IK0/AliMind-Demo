...
- Calculate MAE, RMSE, and RÂ² for TPB construct scoring
...
- Are â‰¥85% of responses rated as nutritionally safe by experts?
...
=== Operationalizing the Framework with Accessible Language Models

The theoretical integration described above places significant demands on the system's natural language understanding capabilities. It must infer subtle psychological constructs (Attitude, Subjective Norm, PBC), detect linguistic markers of stage transitions, select appropriate intervention modes, and generate empathetic, stage-matched responsesâ€”all in real-time conversation. To meet these demands while maintaining accessibility and reproducibility, this study leverages a combination of encoder-only models for inference tasks and decoder-based models in the 7B to 8B parameter range for response generation.
...
#table(
  ...
  [Rule-based policy (simple stageâ†’mode lookup table) or fine-tuned Llama/Mistral with instruction prompts specifying mode per stage @vaswani2024llms @hu2023finetuning],
...
The conversational prompts are derived from the open-ended survey instruments developed during the preliminary phase (Section II). Formal Likert-scale items (e.g., â€œFor me, eating a healthy diet over the next month would be: bad/goodâ€) are reformulated into natural questions that invite free-text responses (e.g., â€œHow do you feel about eating healthier, do you think it would make a difference for you?â€). This transformation preserves the theoretical intent of the original items while making the interaction feel conversational rather than interrogative. The prompts systematically target the three TPB constructs (Attitude, Subjective Norms, Perceived Behavioral Control) and TTM stage indicators, ensuring that the data needed for subsequent inference steps are elicited organically.

Conversation flow is managed using the LangChain framework, which supports prompt chaining and context retention. A dialogue state tracker maintains a representation of which constructs have been sufficiently discussed, which constraints (e.g., time, budget, cooking skills) have been mentioned, and any outstanding clarifications needed. This tracker is updated after each user turn and informs the systemâ€™s next utterance. For example, if the user has not yet provided information about their social environment, the system might ask: â€œDo the people you live with influence what you eat?â€ The tracker also records metadata such as session duration, and turn count that could affect PBC, as highlighted [@bassett-gunterOhBabyMotivation2013].
...
#table(
  ...
  [Attitude (TPB)],
  [Health benefits (e.g., â€œIâ€™d have more energyâ€)],
  [â€œHow do you feel about eating healthier, do you think it would make a difference for you?â€],
  [Instrumental attitude (perceived benefits)],

  [Subjective Norm (TPB)],
  [Peer influence (e.g., â€œmy friends eat out a lotâ€)],
  [â€œHow would your family react if you started preparing different foods?â€],
  [Anticipated social response],

  [Perceived Behavioral Control (TPB)],
  [Time scarcity (e.g., â€œI donâ€™t have time to cookâ€)],
  [â€œWhat gets in the way of eating the way youâ€™d like to, time, money, cooking skills, something else?â€],
  [Barriers and self-efficacy],

  [TTM Stage],
  [Ambivalence (e.g., â€œI know I should, but...â€)],
  [â€œWhere would you say you are with healthy eatingâ€”thinking about it, just starting, or already making changes?â€],
  [Stage classification],
)

The illustrative themes listed above are examples only. Actual themes will be derived from thematic analysis of the preliminary surveyâ€™s open-ended responses. The mapping of themes to TPB constructs will be finalized after the preliminary phase.
...
- $"confidence"_i$ is the classifier's confidence score for theme $i$ (range 0â€“1)
...
The denominator normalizes scores to a 0â€“1 range, which can be scaled to match the original Likert-scale range (1â€“5 or 1â€“7) for consistency with the training data. This formula accounts for both the presence of themes and their relative prominence in the conversation, weighted by population-specific importance and theoretical mapping.
...
and the change persists across at least two consecutive sessions to avoid oscillation. When forward movement is detected (e.g., Contemplation â†’ Preparation), the system may offer positive reinforcement. When backsliding is detected (e.g., Action â†’ Contemplation), the system may explore barriers and offer additional support.
...
#table(
  ...
  [High], [$P_"max" >= 0.7$], [Accept classification; proceed to Step 4], [â€”],

  [Medium],
  [$0.5 <= P_"max" < 0.7$],
  [Accept classification but flag for monitoring; may ask clarification if pattern persists across sessions],
  ["You seem to be thinking about changesâ€”would you say you're planning to start soon or still considering?"],
...
The fourth step translates the user's TTM stage classification from Step 3 into a coarse intervention mode. This mode determines the general therapeutic approach appropriate for the user's current readiness, operationalizing decades of clinical guidelines from the Transtheoretical Model. The mode serves as a high-level directive that structures the subsequent TPB-targeted intervention in Step 5.
...
#table(
  ...
  [Forward movement detected (e.g., Contemplation â†’ Preparation)],
  ...
  [Backsliding detected (e.g., Action â†’ Contemplation)],
...
#table(
  ...
  [Mode Selection Logic], [Rule-based Python dictionary / state machine], [Deterministic stageâ†’mode mapping],
...
#table(
  ...
  ["It sounds like you're feeling pretty okay with how things are, and that's completely valid. You mentioned you sometimes skip breakfastâ€”did you know that eating something in the morning can help with energy and focus throughout the day? Just something to think about. No pressure at all."],

  [Contemplation],
  [9.2 Pros and cons],
  [User says: "I know I should eat better but I love my comfort foods."],
  ["That tension between what we know and what we love is so real. What do you think you'd miss most if you started making some changes? And on the flip side, what might be better if you ate a little differentlyâ€”more energy, maybe, or feeling stronger?"],

  [Preparation],
  [4.1 Instruction on how to perform],
  [User says: "I'm ready to start meal prepping but I don't know where to begin."],
  ["That's excitingâ€”taking that first step! Let's start super simple. Here's a 3-ingredient chicken and veggie sheet pan meal:\n\n1. Chop one chicken breast and one bell pepper into bite-sized pieces\n2. Toss with a little oil and your favorite seasoning\n3. Spread on a baking sheet and bake at 400Â°F for 20 minutes\n\nThat's it! How does that sound as a starting point?"],

  [Action],
  [1.2 Problem solving],
  [User says: "I've been cooking at home but yesterday I was too tired and ordered pizza."],
  ["First, great job cooking at home all those days! One slip doesn't erase progress. Being too tired is a real challengeâ€”what could help on those low-energy days? Maybe having a frozen backup meal, or a super simple recipe like scrambled eggs with veggies that takes 5 minutes?"],

  [Maintenance],
  [1.4 Action planning],
  [User says: "I've been eating well for months but worried about the holidays."],
  ["It's smart to think aheadâ€”that's exactly what helps people maintain changes long-term. Let's make a quick plan: What's one tricky situation you expect? If [situation] happens, then what could you do? Having a specific 'if-then' plan makes it much easier to cope in the moment."],
)
...
- System Usability Scale (SUS): SUS score is computed as:
  ...
  - Sum these adjusted scores and multiply by 2.5 to obtain the SUS score (range 0â€“100)
...
2. Expert validation ($â‰¥ 85%$ of responses rated as nutritionally safe)
